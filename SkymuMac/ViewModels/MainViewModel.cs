...
namespace Skymu.ViewModels
{
	public class MainViewModel : ObservableObject
    {
        ...
        public async Task InitSidebar()
        {
            ...
            Universal.CurrentUser.PropertyChanged += (o, e) =>
            {
                if (e.PropertyName == nameof(User.ConnectionStatus))
                    curContext.Post(_ => new Action(() => { }).Invoke()
        ...
        public void HandleIncoming(MessageBottle e)
        {
            if (e is MessageRecievedBottle eR)
            {
                ...
                if (SelectedConversation?.Identifier == eR.ConversationId)
                {
                    ...
                    if (eR.Item is Message m && m.Author?.Identifier != Universal.CurrentUser?.Identifier && Universal.ActiveViewModel.IsWindowActive) ;// TODO SoundManager.Play("IM");
                }
        ...
        public async Task SendMessage(string text)
        {
            ...
            try
            {
                // TODO SoundManager.Play("IM_SENT");
        ...
        public void ShowAddContactWindow()
        {
            ...
            {
                // TODO new AddContact().ShowWindow();
            }
            else
            {
                // TODO SoundManager.Play("CALL_ERROR1");
            ...
            if (Settings.InformDND != true)
                Window.InvokeOnMainThread(() =>
                {
                    var alert = new NSAlert
                    {
                        ...
                        ShowsSuppressionButton = true // TODO see if this line works?
                    };
                    alert.AddButton("OK");
                    alert.RunModal();
                    /*onClosing: (s, e) =>
        ...
        public async Task SendFile()
        {
            ...
            if (dlg.RunModal() == (uint)NSModalResponse.OK)
            {
                var filePath = dlg.Urls[0];
                string fileName = Path.GetFileName(filePath.LastPathComponent);

                byte[] data = File.ReadAllBytes(filePath.AbsoluteString);
        ...
        */
        ...
        /* TODO public IEnumerable<(string key, string filename)> GetUniqueEmojiList()
        {
